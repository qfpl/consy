...
{-# RULES
"scanl cons"  [~1] forall f a bs . scanl f a bs =
  build (\c n -> a `c` foldr (scanlFB f c) (constScanl n) bs a)

"scanlList cons" [1] forall (f::a->b->a) (a::a) (bs::[b]).
...
"scanl cons text eta" [~2]
  forall a b c. scanl @Data.Text.Text a b c = Data.Text.scanl a b c

"scanl cons text lazy" [~2]
  scanl @Data.Text.Lazy.Text = Data.Text.Lazy.scanl

"scanl cons text lazy eta" [~2]
  forall a b c. scanl @Data.Text.Lazy.Text a b c = Data.Text.Lazy.scanl a b c

"scanl cons bs" [~2]
  scanl @Data.ByteString.ByteString = Data.ByteString.scanl

"scanl cons bs eta" [~2]
  forall a b c. scanl @Data.ByteString.ByteString a b c = Data.ByteString.scanl a b c

"scanl cons bs lazy" [~2]
  scanl @Data.ByteString.Lazy.ByteString = Data.ByteString.Lazy.scanl

"scanl cons bs lazy eta" [~2]
  forall a b c. scanl @Data.ByteString.Lazy.ByteString a b c = Data.ByteString.Lazy.scanl a b c

"scanl cons vector" [~2]
  scanl @(Data.Vector.Vector _) = Data.Vector.scanl

"scanl cons vector eta" [~2]
  forall a b c. scanl @(Data.Vector.Vector _) a b c = Data.Vector.scanl a b c
#-}
...
{-# noinline [1] scanl' #-}
scanl'
  :: (AsEmpty s, AsEmpty t, Cons s s a a, Cons t t b b)
  => (b -> a -> b) -> b -> s -> t
scanl' = scanlGo'
  where
    scanlGo' f !q ls =
...
"scanlList' cons" [1] forall (f::b->a->b) a (bs::[a]) .
...
"scanl1 cons text eta" [~2]
  forall a b. scanl1 @Data.Text.Text a b = Data.Text.scanl1 a b

"scanl1 cons text lazy" [~2]
  scanl1 @Data.Text.Lazy.Text = Data.Text.Lazy.scanl1

"scanl1 cons text lazy eta" [~2]
  forall a b. scanl1 @Data.Text.Lazy.Text a b = Data.Text.Lazy.scanl1 a b

"scanl1 cons bs" [~2]
  scanl1 @Data.ByteString.ByteString = Data.ByteString.scanl1

"scanl1 cons bs eta" [~2]
  forall a b. scanl1 @Data.ByteString.ByteString a b = Data.ByteString.scanl1 a b

"scanl1 cons vector" [~2]
  scanl1 @(Data.Vector.Vector _) = Data.Vector.scanl1

"scanl1 cons vector eta" [~2]
  forall a b. scanl1 @(Data.Vector.Vector _) a b = Data.Vector.scanl1 a b
#-}


{-# NOINLINE [1] scanr #-}
scanr
  :: (AsEmpty s, Cons s s a a, AsEmpty t, Cons t t b b)
  => (a -> b -> b) -> b -> s -> t
scanr f q0 = go
  where
    go a =
...
{-# RULES
"scanr" [~1] forall f q0 (ls::[a]). scanr f q0 ls =
  build (\c n -> strictUncurryScanr c (foldr (scanrFB f c) (q0,n) ls))
"scanrList" [1] forall f q0 (ls::[a]) .
...
"scanr cons text eta" [~2]
  forall a b c. scanr @Data.Text.Text a b c = Data.Text.scanr a b c

"scanr cons text lazy" [~2]
  scanr @Data.Text.Lazy.Text = Data.Text.Lazy.scanr

"scanr cons text lazy eta" [~2]
  forall a b c. scanr @Data.Text.Lazy.Text a b c = Data.Text.Lazy.scanr a b c

"scanr cons bs" [~2]
  scanr @Data.ByteString.ByteString = Data.ByteString.scanr

"scanr cons bs eta" [~2]
  forall a b c. scanr @Data.ByteString.ByteString a b c = Data.ByteString.scanr a b c

"scanr cons vector" [~2]
  scanr @(Data.Vector.Vector _) = Data.Vector.scanr

"scanr cons vector eta" [~2]
  forall a b c. scanr @(Data.Vector.Vector _) a b c = Data.Vector.scanr a b c
#-}
...
"scanr1 cons text eta" [~2]
  forall a b. scanr1 @Data.Text.Text a b = Data.Text.scanr1 a b

"scanr1 cons text lazy" [~2]
  scanr1 @Data.Text.Lazy.Text = Data.Text.Lazy.scanr1

"scanr1 cons text lazy eta" [~2]
  forall a b. scanr1 @Data.Text.Lazy.Text a b = Data.Text.Lazy.scanr1 a b

"scanr1 cons bs" [~2]
  scanr1 @Data.ByteString.ByteString = Data.ByteString.scanr1

"scanr1 cons bs eta" [~2]
  forall a b. scanr1 @Data.ByteString.ByteString a b = Data.ByteString.scanr1 a b

"scanr1 cons vector" [~2]
  scanr1 @(Data.Vector.Vector _) = Data.Vector.scanr1

"scanr1 cons vector eta" [~2]
  forall a b. scanr1 @(Data.Vector.Vector _) a b = Data.Vector.scanr1 a b
#-}
